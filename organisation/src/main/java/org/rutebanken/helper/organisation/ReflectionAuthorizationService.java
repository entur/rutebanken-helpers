...
public class ReflectionAuthorizationService
  implements DataScopedAuthorizationService {

  private static final Logger logger = LoggerFactory.getLogger(
    ReflectionAuthorizationService.class
  );
  ...
  public ReflectionAuthorizationService(
  ...
  public boolean isAuthorized(String requiredRole, Collection<?> entities) {
    ...
    List<RoleAssignment> relevantRoles = roleAssignmentExtractor
      ...
      boolean allowed =
        ...
        relevantRoles
          ...
          .anyMatch(roleAssignment ->
            authorized(roleAssignment, entity, requiredRole)
          );
      if (!allowed) {
        // No need to loop further, if not authorized with required role for one of the entities in collection.
        logger.info(
          "User is not authorized for entity with role: {}. Relevant roles: {}. Entity: {}",
          requiredRole,
          relevantRoles,
          entity
        );
  ...
  public Set<String> getRelevantRolesForEntity(Object entity) {
    return roleAssignmentExtractor
      ...
      .filter(roleAssignment ->
        roleAssignment.getEntityClassifications().get(ENTITY_TYPE) != null
      )
      .filter(roleAssignment ->
        roleAssignment
          ...
          .get(ENTITY_TYPE)
          .stream()
          .anyMatch(entityTypeString ->
            entityTypeString.equalsIgnoreCase(
  ...
  public boolean authorized(
    ...
    if (roleAssignment.getEntityClassifications() == null) {
      logger.warn(
        "Role assignment entity classifications cannot be null: {}",
        roleAssignment
      );
      return false;
    }
    ...
    if (!roleAssignment.getRole().equals(requiredRole)) {
      logger.debug(
        "No role match for required role {}, {}",
        requiredRole,
        roleAssignment
      );
      return false;
    ...
    if (
      !organisationChecker.entityMatchesOrganisationRef(roleAssignment, entity)
    ) {
      logger.debug(
        "Entity does not match organisation ref. RoleAssignment: {}, Entity: {}",
        roleAssignment,
        entity
      );
      return false;
    ...
    if (
      ...
      logger.debug(
        "Entity classification. Not authorized: {}, {}",
        requiredRole,
        roleAssignment
      );
      return false;
    ...
    if (!checkAdministrativeZone(roleAssignment, entity)) {
      logger.debug(
        "Entity type administrative zone no match: {} entity: {}",
        roleAssignment,
        entity
      );
      return false;
  ...
  private boolean checkEntityClassifications(
    ...
    if (!containsEntityTypeOrAll(roleAssignment, entityTypename)) {
      logger.debug(
        "No match for entity type {} for required role {}. Role assignment: {}",
        entity.getClass().getSimpleName(),
        requiredRole,
        roleAssignment
      );
    ...
    for (String entityType : roleAssignment
      .getEntityClassifications()
      .keySet()) {
      boolean authorized = checkEntityClassification(
        entityType,
        entity,
        roleAssignment.getEntityClassifications().get(entityType)
      );

      if (!authorized) {
        logger.info(
          "Not authorized for entity {} and role assignment {}",
          entity,
          roleAssignment
        );
        return false;
  ...
  private boolean checkEntityClassification(
    String entityType,
    Object entity,
    List<String> classificationsForEntityType
  ) {
    ...
    if (
      classificationsForEntityType.contains(ENTITY_CLASSIFIER_ALL_ATTRIBUTES)
    ) {
      logger.debug(
        "Contains {} for {}",
        ENTITY_CLASSIFIER_ALL_ATTRIBUTES,
        entityType
      );
      return true;
    }

    boolean isBlacklist = classificationsForEntityType
      .stream()
      .anyMatch(classifier -> classifier.startsWith("!"));
    boolean isWhiteList = classificationsForEntityType
      .stream()
      .noneMatch(classifier -> classifier.startsWith("!"));

    if (isBlacklist && isWhiteList) {
      logger.warn(
        "The list of classifiers contains values with both black list values (values prefixed with !) and white list values. This is not supported"
      );
      return false;
    }
    ...
    if (mappings != null) {
      ...
      if (isBlacklist) {
        ...
        return mappings
          .stream()
          .allMatch(mappedField ->
            isAllowedForFieldAndClassification(
              mappedField,
              entity,
              classificationsForEntityType,
              true
            )
          );
      } else {
      ...
      return isAllowedForFieldAndClassification(
        entityType,
        entity,
        classificationsForEntityType,
        isBlacklist
  ...
  private boolean isAllowedForFieldAndClassification(
    String entityType,
    Object entity,
    List<String> classificationsForEntityType,
    boolean isBlacklist
    ...
    if (optionalValue.isEmpty()) {
      logger.trace(
        "Cannot resolve value for {}. This is ok, as the value can be null. entity: {}.",
        field,
        entity
      );
      return isBlacklist;
      ...
      isAllowed =
        classificationsForEntityType
          ...
          .anyMatch(classification ->
            removeUnderscore(classification).equalsIgnoreCase(stringValue)
          );
    }

    if (!isAllowed) {
      logger.info(
        "Not allowed value {}: {} for entity {}",
        entityType,
        value,
        entity
      );
  ...
  private Optional<Field> findFieldFromClassifier(
    String classifier,
    Object entity
  ) {
    return Stream
  ...
  private boolean checkAdministrativeZone(
    RoleAssignment roleAssignment,
    Object entity
  ) {
    return (
      roleAssignment.getAdministrativeZone() == null ||
      roleAssignment.getAdministrativeZone().isEmpty() ||
      administrativeZoneChecker.entityMatchesAdministrativeZone(
        roleAssignment,
        entity
      )
    );
  ...
  private boolean containsEntityTypeOrAll(
    RoleAssignment roleAssignment,
    String entityTypeName
  ) {
    List<String> classifiers = roleAssignment
      .getEntityClassifications()
      .get(ENTITY_TYPE);
