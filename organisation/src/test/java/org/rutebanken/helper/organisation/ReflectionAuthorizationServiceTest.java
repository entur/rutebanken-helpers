...
class ReflectionAuthorizationServiceTest {
  ...
  private final RoleAssignmentExtractor roleAssignmentExtractorNoEntityClassification =
    new RoleAssignmentExtractor() {
      @Override
      public List<RoleAssignment> getRoleAssignmentsForUser() {
        List<RoleAssignment> roleAssignments = new ArrayList<>();
        RoleAssignment roleAssignment1 = RoleAssignment
          .builder()
        ...
        RoleAssignment roleAssignment2 = RoleAssignment
          .builder()
          .withRole("editStops")
          .withOrganisation("OST")
  ...
  private final ReflectionAuthorizationService reflectionAuthorizationService =
    new ReflectionAuthorizationService(
      roleAssignmentExtractor,
      true,
      organisationChecker,
      administrativeZoneChecker,
      entityResolver,
      fieldMappings
    );
  private final ReflectionAuthorizationService reflectionAuthorizationServiceNoEntityClassification =
    new ReflectionAuthorizationService(
      roleAssignmentExtractorNoEntityClassification,
      true,
      organisationChecker,
      administrativeZoneChecker,
      entityResolver,
  ...
  void authorizedForLegalStopPlaceTypesWhenOthersBlacklisted() {
    ...
    boolean authorized = reflectionAuthorizationService.authorized(
      roleAssignment,
      stopPlace,
      roleAssignment.r
    );
  ...
  void notAuthorizedWithCombinationOfNegatedTwoFields() {
    ...
    assertThat(
      railReplacementBus + " not allowed for combinedField",
      authorized,
      is(false)
    );
  }
  ...
  void notAuthorizedWithCombinationOfNegatedTwoFields2() {
    ...
    assertThat(
      "airport not allowed for stop place type",
      authorized,
      is(false)
    );
  }
  ...
  void notAuthorizedWithCombinationOfNegatedTwoFields3() {
    ...
    assertThat(
      "railReplacementBus allowed, but not stop place type airport",
      authorized,
      is(false)
    );
  }
  ...
  void handleMappedClassificationsNegation() {
    ...
    fieldMappings.put(
      "anotherproperty",
      Arrays.asList("someField1", "someField2")
    );
  ...
  void handleMappedClassifications() {
    ...
    assertThat(
      "someval2 should be allowed for anotherProperty",
      authorized,
      is(true)
    );
  }
  ...
  void authorizedForLegalSubmodeTypesWhenStarValue() {
    ...
    assertThat(
      reflectionAuthorizationService.authorized(
        roleAssignment,
        stopPlace,
        "editStops"
      ),
  ...
  void notAuthorizedWhenValueEmpty() {
    ...
    assertThat(
      "stop place type is not set, which means that it's not airport",
      authorized,
      is(true)
    );
  }
  ...
  void multipleEnumsShouldBeAllowed() {
    ...
    roleAssignment
      ...
      .put(
        "StopPlaceType",
        types.stream().map(Enum::toString).collect(toList())
      );
    ...
    types.forEach(type -> {
      ...
      assertThat(
        "Should have access to edit stop with type " + type,
        authorized,
        is(true)
      );
    });
  ...
  void mixNegationForEnums() {
    RoleAssignment roleAssignment = RoleAssignment
      ...
      .withEntityClassification(
        "StopPlaceType",
        StopPlace.StopPlaceType.AIRPORT.toString()
      )
      .withEntityClassification(
        "StopPlaceType",
        "!" + StopPlace.StopPlaceType.ONSTREET_BUS
      ...
      .withEntityClassification(
        "StopPlaceType",
        StopPlace.StopPlaceType.ONSTREET_TRAM.toString()
      )
      .build();
  ...
  void authorizedForCertainEnumValues() {
    for (StopPlace.StopPlaceType enumValue : StopPlace.StopPlaceType.values()) {
      ...
      assertThat(
        "should not be authorized as negation is on !",
        authorized,
        is(false)
      );

      Arrays
        .stream(StopPlace.StopPlaceType.values())
        .forEach(otherEnum -> {
          if (otherEnum != enumValue) {
            stopPlace.stopPlaceType = otherEnum;
            assertThat(
              "One value is not allowed " +
              enumValue +
              ", but " +
              otherEnum +
              " is",
  ...
  void getRelevantRolesForEntityTest() {
    ...
    stopPlace.someField1 = null;
    stopPlace.someField2 = "somethingElse";
    final Set<String> relevantRolesForEntity =
      reflectionAuthorizationServiceNoEntityClassification.getRelevantRolesForEntity(
        stopPlace
      );
