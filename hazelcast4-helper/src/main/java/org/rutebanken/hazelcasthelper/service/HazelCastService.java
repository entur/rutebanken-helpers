...
public class HazelCastService {
  ...
  public final void init() {
    if (kubernetesService != null && kubernetesService.isKubernetesEnabled()) {
      ...
      try {
        ...
        throw new RutebankenHazelcastException(
          "Could not run initialization of hazelcast.",
          e
        );
      }
    } else {
    ...
    if (shutDownHookEnabled) {
      Runtime
        .getRuntime()
        .addShutdownHook(new Thread(() -> hazelcast.shutdown()));
    }
  }
  ...
  private HazelcastInstance runHazelcast(String groupName) {
    ...
    log.info(
      "Old config: b_count {} async_b_count {} read_backup_data {}",
    ...
    log.info(
      "Updated config: b_count {} async_b_count {} read_backup_data {}",
      mapConfig.getBackupCount(),
      mapConfig.getAsyncBackupCount(),
      mapConfig.isReadBackupData()
    );
    ...
    getSerializerConfigs()
      .forEach(cfg.getSerializationConfig()::addSerializerConfig);
  ...
  private HazelcastInstance initForLocalHazelCast() {
    log.info(
      "Running hazelcast with LOCAL configuration ONLY - this is for junit tests and when you do not have a kubernetes environment"
    );
    final Config cfg = new Config()
      .setInstanceName(UUID.randomUUID().toString())
      .setProperty("hazelcast.phone.home.enabled", "false");
