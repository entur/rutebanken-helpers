...
public class BlobStoreHelper {
  ...
  public static Iterator<Blob> listAllBlobsRecursively(
    Storage storage,
    String containerName,
    String prefix
  ) {
    LOGGER.debug(
      "Listing blobs in bucket {} with prefix {} recursively.",
      containerName,
      prefix
    );
  ...
  public static Blob uploadBlob(
    ...
    LOGGER.debug(
      "Stored blob with name '{}' and size '{}' in bucket '{}'",
      blob.getName(),
      blob.getSize(),
      blob.getBucket()
    );
   ...
   */
  @Deprecated
  public static Blob uploadBlobWithRetry(
    Storage storage,
    String containerName,
    String name,
  ...
  public static Blob uploadBlobWithRetry(
    ...
    LOGGER.debug(
      "Uploading with retry blob {} to bucket {}",
      name,
      containerName
    );
    BlobId blobId = BlobId.of(containerName, name);
    ...
    try {
      ...
      throw new org.rutebanken.helper.storage.BlobStoreException(
        "Blob upload of blob with name '" +
        blobId.getName() +
        "' failed: " +
        ioE.getMessage(),
        ioE
   ...
   */
  private static void writeWithRetry(
    Storage storage,
    BlobInfo blobInfo,
    InputStream inputStream
  ) throws IOException {
    ...
    try {
      ...
      if (e.getCode() == HttpStatusCodes.STATUS_CODE_PRECONDITION_FAILED) {
        throw new BlobAlreadyExistsException(
          "The blob with name '" + blobId.getName() + "' already exists",
          e
        );
      } else {
        throw new org.rutebanken.helper.storage.BlobStoreException(
   ...
   * @throws org.rutebanken.helper.storage.BlobStoreException            if the blob creation fails.
   */
  public static Blob createOrReplace(
    Storage storage,
    String containerName,
    String name,
    InputStream inputStream,
  ...
  public static Blob createOrReplace(
    ...
    LOGGER.debug(
      "Creating or replace blob {} in bucket {}",
      name,
      containerName
    );
    BlobId blobId = BlobId.of(containerName, name);
    ...
    if (existingBlob == null) {
      ...
      BlobInfo.Builder builder = BlobInfo
        .newBuilder(existingBlob.getBlobId())
        .setContentType(contentType);
      if (makePublic) {
        builder.setAcl(List.of(Acl.of(Acl.User.ofAllUsers(), Acl.Role.READER)));
        builder.setCacheControl(DEFAULT_CACHE_CONTROL);
      ...
      try {
        ...
        if (e.getCode() == HttpStatusCodes.STATUS_CODE_PRECONDITION_FAILED) {
          throw new BlobConcurrentUpdateException(
            "The blob with name '" +
            blobId.getName() +
  ...
  public static boolean existBlob(
    Storage storage,
    String containerName,
    String name
  ) {
    LOGGER.debug(
      "Checking that blob {} from bucket {} exists",
      name,
      containerName
    );
    BlobId blobId = BlobId.of(containerName, name);
  ...
  public static InputStream getBlob(
    Storage storage,
    String containerName,
    String name
  ) {
    ...
    if (blob != null) {
      LOGGER.debug(
        "Retrieved blob with name '{}' and size '{}' from bucket '{}'",
        blob.getName(),
        blob.getSize(),
        blob.getBucket()
      );
  ...
  public static boolean deleteBlobsByPrefix(
    Storage storage,
    String containerName,
    String prefix
  ) {
    List<BlobId> blobIdList = new ArrayList<>();
    listAllBlobsRecursively(storage, containerName, prefix)
      .forEachRemaining(blob -> blobIdList.add(blob.getBlobId()));
  ...
  public static Storage getStorage(String projectId) {
    ...
    HttpTransportOptions transportOptions = HttpStorageOptions
      .defaults()
      .getDefaultTransportOptions();
    transportOptions =
      transportOptions
        .toBuilder()
        .setConnectTimeout(CONNECT_AND_READ_TIMEOUT)
        .setReadTimeout(CONNECT_AND_READ_TIMEOUT)
        .build();
