...
class BlobStoreHelperTest {
  ...
  private static final String credentialPath =
    "/home/tomgag/.ssh/Carbon-ef49cabc6d04.json";
  ...
  void beforeEach() {
    ...
    blobIterator.forEachRemaining(blob ->
      BlobStoreHelper.delete(storage, BlobId.of(BUCKET_NAME, blob.getName()))
    );
  }
  ...
  void testDownload() throws IOException {
    ...
    InputStream inputStream = BlobStoreHelper.getBlob(
      storage,
      BUCKET_NAME,
      blobName
    );
    ...
    byte[] expected = ByteStreams.toByteArray(
      this.getClass().getResourceAsStream(fileName)
    );
