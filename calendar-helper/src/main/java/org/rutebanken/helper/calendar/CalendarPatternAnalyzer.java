...
public class CalendarPatternAnalyzer {
  ...
  public CalendarPattern computeCalendarPattern(Set<LocalDate> includedDays) {
    ...
    if (!significantDays.isEmpty()) {
      TreeSet<LocalDate> sortedMatchingDays = includedDays
        .stream()
        .filter(d -> significantDays.contains(d.getDayOfWeek()))
        .collect(Collectors.toCollection(TreeSet::new));
      if (!sortedMatchingDays.isEmpty()) {
        return buildCalendarPattern(
          includedDays,
          significantDays,
          sortedMatchingDays
        );
   ...
   */
  public Set<DayOfWeek> computeSignificantDays(
    LocalDate d,
    boolean[] included
  ) {
    Map<DayOfWeek, WeekDayEntry> dayMap = initWeekDayMap();
  ...
  private CalendarPattern buildCalendarPattern(
    Set<LocalDate> includedDays,
    Set<DayOfWeek> significantDays,
    TreeSet<LocalDate> sortedMatchingDays
  ) {
    LocalDate patternStart = calculateStartOfValidityIntervalInclusive(
      sortedMatchingDays.first(),
      significantDays
    );
    LocalDate patternEnd = calculateEndOfValidityIntervalExclusive(
      sortedMatchingDays.last(),
      significantDays
    );

    Set<LocalDate> additionalDates = includedDays
      .stream()
      .filter(md -> md.isBefore(patternStart) || md.isAfter(patternEnd))
  ...
  private Set<DayOfWeek> computeSignificantDays(
    Map<DayOfWeek, WeekDayEntry> dayMap
  ) {
    ...
    if (totalDaysIncluded > MIN_DAYS_FOR_PATTERN) {
      for (WeekDayEntry entry : dayMap.values()) {
        entry.setPercentage(
          ((double) entry.getCount()) * 100 / (double) totalDaysIncluded
        );
      }
      ...
      for (int i = 1; i <= DAYS_PER_WEEK; i++) {
        double minDayPercentage = (double) (
          MIN_PERCENTAGE_ALL_DAYS_DETECTED - ERROR_MARGIN
        ) /
        (double) i; // for i=2 this means 42.5 for each day type
        ...
        if (
          allDaysAboveMinDayPercentage &&
          totalDayPercentage > MIN_PERCENTAGE_ALL_DAYS_DETECTED
        ) {
          for (int j = 0; j < i; j++) {
      ...
      log.debug(
        "Too few days to extract pattern, expected at least {} but only got {}",
   ...
   */
  private LocalDate calculateStartOfValidityIntervalInclusive(
    LocalDate firstMatchingDay,
    Set<DayOfWeek> expectedDays
  ) {
    for (int i = 1; i <= DAYS_PER_WEEK; i++) {
   ...
   */
  private LocalDate calculateEndOfValidityIntervalExclusive(
    LocalDate lastMatchingDay,
    Set<DayOfWeek> expectedDays
  ) {
    for (int i = 1; i <= DAYS_PER_WEEK; i++) {
  ...
  private Map<DayOfWeek, WeekDayEntry> initWeekDayMap() {
    return Arrays
      ...
      .map(WeekDayEntry::new)
      .collect(Collectors.toMap(WeekDayEntry::getDayType, Function.identity()));
  }
